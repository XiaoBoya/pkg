...
func (a *gitBranchLister) Register(ws *restful.WebService) {
	...
	ws.Route(
		ws.GET("/repositories/{repository}/branches").To(a.ListBranch).
			Doc("ListBranch").Param(repositoryParam).
...
func (a *gitBranchLister) ListBranch(request *restful.Request, response *restful.Response) {
	...
	branchList, err := a.impl.ListGitBranch(request.Request.Context(), metav1alpha1.GitRepo{Repository: repo}, option)
	if err != nil {
...
func (a *gitBranchCreator) Register(ws *restful.WebService) {
	repositoryParam := ws.PathParameter("repository", "branch belong to repository")
	ws.Route(
		ws.POST("/repositories/{repository}/branches").To(a.CreateBranch).
			Doc("CreateBranch").Param(repositoryParam).
			Metadata(restfulspec.KeyOpenAPITags, a.tags).
...
func (a *gitBranchCreator) CreateBranch(request *restful.Request, response *restful.Response) {
	...
	payload := metav1alpha1.CreateBranchPayload{GitRepo: metav1alpha1.GitRepo{Repository: repo}, CreateBranchParams: params}
