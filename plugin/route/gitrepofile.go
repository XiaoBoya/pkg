...
func (a *gitRepoFileGetter) Register(ws *restful.WebService) {
	...
	ws.Route(
		ws.GET("/repositories/{repository}/contents/{path}").To(a.GetGitRepoFile).
			Doc("GetGitRepoFile").Param(repositoryParam).Param(pathParam).Param(refParam).
...
func (a *gitRepoFileCreator) Register(ws *restful.WebService) {
	...
	ws.Route(
		ws.POST("/repositories/{repository}/content/{filepath}").To(a.CreateGitRepoFile).
			Doc("CreateBranch").Param(repositoryParam).Param(pathParam).
			Metadata(restfulspec.KeyOpenAPITags, a.tags).
